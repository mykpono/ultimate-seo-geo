...
1. **Check AI crawler access** — Fetch `/robots.txt`. Confirm the search crawlers OAI-SearchBot, Claude-SearchBot and PerplexityBot are not Disallowed; they build the indexes that AI answers cite. GPTBot and ClaudeBot are training crawlers, so blocking them is a licensing choice, not a visibility defect. Then run `scripts/ai_bot_access.py` on the URL: a firewall or CDN bot rule can refuse a crawler that robots.txt allows. The test cannot use the vendor's IP ranges, so report a refusal as a suspected block and confirm it in the firewall logs before recommending a rule change. Where the site owner can share access logs, run `scripts/ai_bot_logs.py` with `--verify-ips`: logs are the only record of what the real crawlers requested and what the server answered (see `references/crawl-indexation.md` → AI Crawlers in Logs).
...
3. **Score citability** — For each key page: does the first 40–60 words answer the target query? Self-contained 134–167 word answer blocks? Question-format headings? Pages already optimized for Featured Snippets (§ 7c) have a structural head-start on AI citation.
   - **Measure the structure** with `python scripts/citability_checker.py --url https://example.com/guide --json` (the full report runs it on the audited page). It scores prose walls over 350 words, paragraphs over 120 words, the heading outline and specific figures, and reports whether a lead paragraph starts within the first 60 words. It cannot tell whether a passage answers the query: judge that yourself. Homepages, section fronts and hubs are "not applicable"; run it on article, guide and docs URLs. Its score is shown in the report but not weighted.
   - **Write fixes for readers.** Lead with the answer and split walls because it makes the page clearer; never add text aimed at AI systems (§ 19 rules 10c, 10d).
...
4. **Check JavaScript rendering** — Fetch raw page source. Key content absent from raw HTML = invisible to AI bots.
   ...
   - **Hidden instructions to AI systems.** Run `scripts/hidden_instructions.py` on key pages; the full report runs it on the audited page. Instruction-like text hidden from visitors is Critical. It is either a compromise to clean up or spam under Google's policy on manipulating generative AI responses. See `references/ai-search-geo.md` → Hidden Instructions to AI Systems.
...
7. **Test AI citation presence** — Measure it as a rate, not a single search: AI answers change between runs. Run a fixed set of 10–30 user-phrased prompts at least 5 times per engine, in fresh sessions. Record whether the site was cited and which domains were, then score the runs with `scripts/citation_sampling.py` (95% Wilson intervals). Report an "inconsistent" result as needing more runs, and never call a change real when the intervals overlap. With the answers pasted into the `answer` column, `--facts brand.json` also checks them for wrong brand facts (founding year, HQ, founders, prices, claims) and names the page to correct. Method: `references/ai-search-geo.md` → Measuring AI Citation Presence.
