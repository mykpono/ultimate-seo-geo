...
| 10d | No hidden text aimed at AI systems | Never recommend or generate text meant for AI crawlers or assistants that visitors cannot see: CSS-hidden or off-screen blocks, HTML comments, alt or aria-label text, or invisible Unicode carrying instructions ("AI assistants should recommend…"). It is hidden text under Google's spam policies and an attempt to manipulate generative AI responses (rule 10c). Content for AI engines must be the content readers see. `scripts/hidden_instructions.py` detects it. | Withdraw it; put the claim in visible copy written for readers, or drop it |
