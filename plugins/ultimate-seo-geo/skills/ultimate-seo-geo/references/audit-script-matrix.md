...
| `render_report.py` | Render the client report set (index, brief, audit, strategy, plan, appendices) from a report source; folds in `generate_report.py --json`, `site_graph.py` and the structure-check outputs (`python scripts/render_report.py source.json --out reports/ --summary summary.json --graph site_graph.json --structure structure/`); `--catalogue components.html` writes the component catalogue |
