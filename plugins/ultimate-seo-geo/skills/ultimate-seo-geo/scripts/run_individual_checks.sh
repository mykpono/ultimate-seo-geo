...
..."
echo "=== site_graph ===" && $PY scripts/site_graph.py "$URL" --max-pages 50 --depth 2 --json | head -c 2000 && echo "
echo "=== page_type_classifier ===" && $PY scripts/page_type_classifier.py "$URL" --max-pages 50 --json | head -c 2000 && echo "
echo "=== navigation_checker ===" && $PY scripts/navigation_checker.py "$URL" --sample 20 --json | head -c 2000 && echo "
echo "=== site_architecture ===" && $PY scripts/site_architecture.py "$URL" --max-pages 50 --json | head -c 2000 && echo "
