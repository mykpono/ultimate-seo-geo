...
> The definitive SEO + GEO skill for Claude. Full site audits, AI search optimization, schema generation, E-E-A-T assessment, and **51** bundled diagnostic Python scripts.
...
**51** audit `.py` scripts in `scripts/` (maintainer-only checkers are not bundled). Full step ↔ script map: **`references/audit-script-matrix.md`**. Eval regression: `evals/fixtures/` + `score_eval_transcript.py --all-fixtures`.
